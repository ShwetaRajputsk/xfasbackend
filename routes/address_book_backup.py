...
async def create_saved_address(
    ...
    try:
        ...
        logger.error(f"Failed to create address for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
...
async def get_user_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        addresses = await address_book_service.get_user_addresses(
            user_id=user_id,
        ...
        logger.error(f"Failed to get addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
...
async def get_address_by_id(
    ...
    try:
        user_id = current_user["user_id"]
        address = await address_book_service.get_address_by_id(user_id, address_id)
        ...
        logger.error(f"Failed to get address {address_id} for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_address(
    ...
    try:
        user_id = current_user["user_id"]
        address = await address_book_service.update_address(user_id, address_id, update_data)
        ...
        logger.error(f"Failed to update address {address_id} for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
...
async def delete_address(
    ...
    try:
        user_id = current_user["user_id"]
        success = await address_book_service.delete_address(user_id, address_id)
        ...
        logger.error(f"Failed to delete address {address_id} for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bulk_delete_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        deleted_count = await address_book_service.bulk_delete_addresses(user_id, request.address_ids)
        ...
        logger.error(f"Failed to bulk delete addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def set_default_address(
    ...
    try:
        user_id = current_user["user_id"]
        success = await address_book_service.set_default_address(
        ...
        logger.error(f"Failed to set default address {address_id} for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def search_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        addresses = await address_book_service.search_addresses(
            user_id=user_id,
            query=query,
            address_type=address_type,
            category=category,
        ...
        logger.error(f"Failed to search addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_address_book_summary(
    ...
    try:
        user_id = current_user["user_id"]
        summary = await address_book_service.get_address_book_summary(user_id)
        return summary
    except Exception as e:
        logger.error(f"Failed to get address book summary for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_default_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        defaults = await address_book_service.get_default_addresses(user_id)
        return defaults
    except Exception as e:
        logger.error(f"Failed to get default addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def mark_address_used(
    ...
    try:
        user_id = current_user["user_id"]
        success = await address_book_service.increment_usage(user_id, address_id)
        ...
        logger.error(f"Failed to mark address {address_id} as used for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def import_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        result = await address_book_service.import_addresses(
            user_id=user_id,
        ...
        logger.error(f"Failed to import addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
...
async def export_addresses(
    ...
    try:
        user_id = current_user["user_id"]
        addresses = await address_book_service.export_addresses(user_id)
        ...
        logger.error(f"Failed to export addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_default_pickup_address(
    ...
    try:
        ...
        logger.error(f"Failed to get default pickup address for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_default_delivery_address(
    ...
    try:
        ...
        logger.error(f"Failed to get default delivery address for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_recent_addresses(
    ...
    try:
        ...
        logger.error(f"Failed to get recent addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_frequent_addresses(
    ...
    try:
        ...
        logger.error(f"Failed to get frequent addresses for user {current_user.get('user_id')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
